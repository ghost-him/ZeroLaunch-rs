...
0. **安全检查**：
   - 如果用户执行默认逻辑：执行默认模式前，先检测工作区是否有未暂存文件：
     - 运行 git status --porcelain，若存在 M（已修改未暂存）且数量 > 0，通过 `AskUserQuestion` 工具，以询问用户的方式，强制输出警告：“⚠️ 检测到工作区存在未暂存的变更。当前将生成【所有本地变更】的摘要，若仅需提交暂存区，请使用 /summarize-changes --staged。”
     - 若用户仍选择继续，则继续执行；若用户选择中止，则退出流程。
