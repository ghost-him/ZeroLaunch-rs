...
- **优雅与精简优先**：代码面向下一位维护者。优先扩展现有抽象；在引入新模块、trait 或兼容层之前，先确认现有架构确实无法容纳需求。
- **不要过度设计**：小功能用最小实现，避免为一个简单需求写一大堆代码。不需要的抽象层、trait、泛型或配置项一律不引入；只有现有架构确实无法容纳需求时才扩展。
- **不要过度容错，该 panic 就 panic**：判断标准是问题成因。问题源于程序员（内部不变量被破坏、处理逻辑写错）→ 直接 `panic!`，让问题第一时间暴露、可被定位；问题与用户操作相关（非法输入、外部环境不可控）→ 在入口边界**第一时间兜底**（校验、降级、报错），但兜底之后的内部处理链路不再兜底、出错即 panic——内部链路 panic 意味着"对该用户操作的处理逻辑写错了"，正是程序员需要排查的信号。过度兜底会掩盖 bug，让问题难以发现与定位。
- **不要过度防御**：防御性检查只放在入口边界（用户输入、外部环境），内部链路信任已建立的不变量。不为"契约保证不可能发生"的分支写兜底代码，不在每个调用点重复校验同一不变量——那会掩盖契约错误、拖慢热路径，让代码噪音淹没真实逻辑。
- **该 debug_assert 的时候就 debug_assert**：内部不变量、索引边界、状态一致性等"程序员写错才会触发"的条件，用 `debug_assert!` 表达——开发与测试环境立即暴露错误，release 编译为零开销。不要因为"release 会被编译掉"就省略检查或改写成手写 if；也不要为这类内部条件引入带错误处理的运行时检查（那是把程序员错误当用户输入兜底）。仅当条件在 release 也必须成立且无法由类型系统保证时，才用运行时检查。
- **禁止为历史遗留加兼容层**：不为老旧代码或旧契约做 shim、别名、过渡字段、补丁函数。历史契约与现状不匹配时**直接修正契约**（前后端同一 commit 同步），不留双轨并存。
- **契约与数据源同构**：跨层事件/载荷直接透传真实数据形状，禁止启发式中间映射（反例：把文件系统事件猜成"应用安装/卸载"再下发——应下发真实事件（kind + 路径），由消费方自行判断）。
- **注释只解释当前代码的作用**：注释仅描述代码做什么（行为、参数、返回、流程），不记录决策动机、历史变更、备选方案；精简、准确，与实现一致。
- **死代码即删**：无用代码立即删除，Git 历史是唯一备份（详见 `.omp/RULES.md` 死代码纪律）。
