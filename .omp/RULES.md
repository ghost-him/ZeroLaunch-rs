...
- 前端允许**展示层派生与输入交互层判定**：基于当前输入与后端已下发数据计算展示/时序状态（computed 派生、防抖豁免、在途提示）——仅影响展示与时序、不产生权威结果、随时可被后端响应推翻。此类判定若镜像后端解析规则，参数 MUST 取后端下发（如 `panel-interaction` 推送的触发词列表），镜像点必须在注释中声明并随后端解析同步更新。
