...
- **业务逻辑的判据是权威性**：产生权威数据、裁决路由/会话状态、持久化、调度平台操作的逻辑属于业务逻辑，MUST 在后端。后端负责 **数据持久化** 与 **逻辑控制**。所有平台操作（文件读写、程序启动、系统调用）MUST 通过 IPC 命令委托给后端。
- 前端允许**展示层派生与输入交互层判定**：基于当前输入与后端已下发数据计算展示/时序状态（computed 派生、防抖豁免、在途提示）——仅影响展示与时序、不产生权威结果、随时可被后端响应推翻。此类判定若镜像后端解析规则，参数 MUST 取后端下发（如 `session-state` 事件推送的触发词列表），镜像点必须在注释中声明并随后端解析同步更新。
...
- **正确**：`queryStillInPanel` / `hasTranslateContent` 据后端推送触发词与镜像最小解析判定防抖/提示时序（权威路由与解析在后端）。**错误**：前端直接写剪贴板（须经 `bridge_confirm` → 后端 `PluginHandle::set_clipboard_text`）。
...
- 注释/错误消息语言定则：宿主（`src-tauri/`）与内置插件中文；平台实现 crate（`crates/platform-windows/`、`crates/platform-macos/`）注释与错误消息可为英文（macos crate 已全英文，windows crate 中文为主——对齐相邻平台 crate 风格即可）。错误消息语言与注释语言同规则，无独立约束。
