...
condition: "impl Plugin for|PluginContext|Arc<PluginHandle>|init_all"
...
- **内置插件**（进程内、生命周期与宿主一致）：允许在 `init()` 中保存 `Arc<PluginHandle>` 到自身状态，供 `query`/`execute_action` 经句柄访问平台能力（如剪贴板）。宿主 MUST 在启动注册完成后对全部内置插件调用 `init()`（`PluginService::init_all`），确保句柄发放
