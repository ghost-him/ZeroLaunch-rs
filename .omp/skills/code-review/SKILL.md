...
5. **架构行为（4b）** — 职责域解耦、通信方式、接口复用（P4/P5/P6，纯人工判断）
...
| 4b — 架构行为  | P4 职责域解耦/P5 通信/P6 复用 | 阻塞             |
...
按 `references/report-template.md` 规定的结构与输出风格生成最终报告。模板包含：总体结论、变更概览、问题清单（统一列表按严重程度分组）、规则不一致、冲突复核、大 commit 下钻、疑点、既有问题、手动验证清单。
...
- **文件内容**: 包含完整的结构化审查结论（`references/report-template.md` 的所有章节），从"总体结论"到"建议手动验证清单"，各子 agent 的报告摘要可精简纳入而不丢失关键信息。文件开头加一行元数据（格式见 `references/report-template.md` 末尾）
