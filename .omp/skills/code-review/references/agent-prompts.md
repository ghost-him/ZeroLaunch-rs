...
**职责**：检查模块间的运行时行为是否合规——职责域有没有直接互调？通信方式对不对？有没有绕过既有抽象造轮子？**有没有引入本次需求不需要的过度设计？** 覆盖 P4/P5/P6 + 过度设计检测，纯人工判断，需设计推理。
...
■ 过度设计检测（中/低·人工，标注「设计债」）
- 定义：为「可能到来的未来」提前支付的抽象成本——本次需求不需要、但作者预期将来需要而引入的机制
- 检测信号：
  1. 零调用者/零消费：新增接口/事件/通道/字段/枚举变体在本次变更后没有任何运行时消费者（前端零读取、IPC 零调用、方法零调用方）
  2. 冗余存储：同一信息在两条以上路径维护（可由其他字段推导、id 双存储）
  3. 预留字段/变体：值为恒定常量的字段（如恒 "main" 的 panel_id）、为未来预留的枚举变体/参数/通道
  4. 状态空间虚胖：声明的合法组合数显著大于实际使用组合数，非法组合靠编码纪律维持（编译器不拦）
  5. 首次使用即踩坏既有语义：新机制的第一个消费者就要绕开或改写既有行为（机制与现状冲突的信号）
  6. 单实现者抽象：新 trait/枚举仅一个实现者且无明确扩展计划
- 判定标准：本次需求是否真实需要该抽象？抽象减少还是增加了认知负担？删除它是否只影响「未来可能」的功能？
- 输出规则：
  - 此类问题定级「中/低」，统一标注「设计债」；**不得**因"为未来好"而升为阻塞
  - 若该抽象实际破坏了现有功能（死键、路由失效、契约漂移），归 Agent 2/3 的阻塞/高问题，不在此列
  - 有明确近期计划（如已排期的第三方插件需求）、抽象成本极低且删除成本更高时，列入「允许保留的设计选择」而非问题
  - 过度设计不同于 P4/P5/P6 违规：前者是"多余的抽象"，后者是"错误的抽象"；发现后按本块处理，不重复表述
...
输出格式：
## Agent 4b 报告 — 架构行为（P4/P5/P6 + 过度设计）
### 发现
...
### 过度设计（设计债）
- **<严重程度: 中/低>** | <文件:行或符号> | <现象>
  - 信号: <命中了哪条检测信号（零调用者/冗余存储/预留/虚胖/踩坏语义/单实现者）>
  - 为何本次需求不需要: <去掉它影响什么——只影响"未来可能"还是影响当前>
  - 建议: <删除/收敛/记录为设计债等待时机>
### 既有架构问题（非本次引入）
- <问题> | <对应原则> | <为何判断为既有>
### 允许保留的设计选择
