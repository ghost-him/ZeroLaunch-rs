...
plugin/             → 插件实现
plugin_system/      → 插件框架（SessionRouter, Registry, Pipeline）
plugin_loader/      → 第三方插件加载（discovery, loader）
cli_server/         → 本地 HTTP API 服务器（axum）
plugin_protocol_assets/ → zlplugin:// 自定义协议处理器
commands/           → IPC 命令（bridge, config, plugin, resource, inspector）
```
...
插件管理（`plugin_` 前缀，4个）：`plugin_list`, `plugin_get_manifest`, `plugin_reload`, `plugin_uninstall`
