...
- **国际化 i18n** — 实现 i18n 框架，至少覆盖中英文
- **右键菜单 ContextMenu 集成** — 组件已存在但未在结果项中使用
- **虚拟滚动优化** — ResultList 在大结果集时使用虚拟滚动
- **错误处理完善** — 统一错误包装层，前端全局错误提示，后端错误日志
- **useKeyboard 增强** — 插件模式下的键盘事件处理
- **EverythingPlugin 后端实现** — 目前为空文件，需接入 Everything SDK
- **搜索管道动态重建** — SearchEngine/ScoreBooster 配置变更时动态更新管道（session_router.rs 中有 TODO）
...
8. **Theme 已持久化** — `theme-store.ts` 从 `localStorage` 读取初始主题，fallback 系统 `prefers-color-scheme`。`setTheme()` 时同步写入 `localStorage`。
