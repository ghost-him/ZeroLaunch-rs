...
### 🎨 个性化与交互
*   **高度自定义外观**：支持自定义背景、颜色、字体、圆角、窗口尺寸等，并提供便捷的调节按钮。
*   **多语言界面**：支持简体中文、繁體中文与英文，自动匹配系统语言。
...
- **版本检查器** — 通过 GitHub Releases 检查并提示新版本
...
*   **第三方插件安装与加载**：当前版本暂不支持第三方插件的安装与重载（页面入口已置灰并提示），需要后续调试与优化；已安装插件的启停、卸载、日志与配置不受影响。
*   **待补充功能**（规划中）：
    *   日志导出为 ZIP 压缩包
    *   Acrylic / Mica / Tabbed 毛玻璃窗口特效（当前仅支持透明背景与圆角）
    *   按住 `Alt` 键临时查看最近启动程序
    *   右键菜单「屏蔽该结果」
    *   参数面板的实时命令行预览
*   **已放弃功能**（不再计划实现）：
...
当前支持：简体中文 (zh-Hans)、繁體中文 (zh-Hant)、English (en)。
