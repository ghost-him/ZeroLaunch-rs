...
/// 查询来源通道 —— 标识查询进入后端的入口。
///
/// 仅 GUI 通道参与会话写入与查询版本域：会话写入由入口结构保证
/// （宿主的 GUI 查询入口是唯一的会话写点），版本域（`QueryRevisionGate`）
/// 同样只覆盖 GUI 查询与热键唤醒；CLI 与面板通道为只读辅助路径 ——
/// 不改写会话、不分配版本号、结果不作废。
///
...
/// 查询版本门控：宿主在会写会话的查询入口（GUI 查询、热键唤醒）分配
/// 单调递增版本号，供插件在写入跨查询共享状态（如翻译结果缓存）前判断
/// 自身查询是否已被更新的查询取代。
///
/// 只读入口（CLI/面板查询）不分配版本号，其结果不作废。
