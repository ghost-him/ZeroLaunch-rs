...
ZeroLaunch 是一款專為 Windows 平台精心打造的應用程式啟動器，**幫你瞬間找到並啟動所需應用，體驗極致高效與快捷！**
...
### ⚡ 高效智能搜尋
得益於獨創搜尋演算法的優化，程式在三重匹配技術（全稱/拼音/首字母）基礎上具備了卓越的拼寫糾錯能力。程式支援中英文混合查詢，結合即時動態排序演算法與多執行緒並發處理技術，即使在配置較低的裝置上也能實現毫秒級響應速度。

### 🌐 輕巧純粹
專注於應用程式搜尋與啟動功能，實現高度特化的應用搜尋。不被其他複雜的功能所打擾，開箱即用，就是這麼純粹。

## 🔬 軟體功能

### 核心功能

*   **應用程式搜尋**：快速檢索並啟動**應用程式**及**UWP應用**，提供流暢的程式存取體驗。
...
*   **偵錯功能**：可以查看程式在當前電腦上的運行情況（基本都沒問題），查看搜尋演算法的運行結果。
...
建置產物路徑：`./src-tauri/target/release/`

## 📦 資料目錄結構

程式的配置檔案分別為：**本地配置檔案**與**遠端配置檔案**，這兩個都是 JSON 格式的檔案。本地配置檔案的存放地址如下，本地配置檔案中存放著遠端配置檔案的地址。遠端配置檔案就是程式運行時所產生的檔案資訊，預設的存放地址也是該目錄。

```
%APPDATA%\ZeroLaunch-rs\                # 例如：C:\Users\[使用者名稱]\AppData\Roaming\ZeroLaunch-rs\
├── logs/                               # 運行日誌
