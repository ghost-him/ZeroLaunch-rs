    ...
    fn setting_schema(&self) -> Vec<SettingDefinition> {
        vec![
            ...
            SchemaBuilder::select(
                "input_source",
                t_key!("symbol-remover", "fields.input_source.label"),
                t_key!("symbol-remover", "fields.input_source.desc"),
            )
            .options_with_labels(&[
                (
                    SOURCE_ORIGINAL_NAME,
                    t_key!("symbol-remover", "options.input_source.original_name"),
                ),
                (
                    SOURCE_NORMALIZED_BASE,
                    t_key!("symbol-remover", "options.input_source.normalized_base"),
                ),
                (
                    SOURCE_REFINED,
                    t_key!("symbol-remover", "options.input_source.refined"),
                ),
                (
                    SOURCE_OPTIMIZER_OUTPUT,
                    t_key!("symbol-remover", "options.input_source.optimizer_output"),
                ),
            ])
            ...
            SchemaBuilder::text(
                "producer_id",
                t_key!("symbol-remover", "fields.producer_id.label"),
                t_key!("symbol-remover", "fields.producer_id.desc"),
            )
            .default("")
            .order(2)
